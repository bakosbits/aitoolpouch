...
export async function getFeaturedTools(count = 4) {
    ...
    console.log(`--- [getFeaturedTools] START: Attempting to fetch ${count} random featured tools ---`);
    ...
    try {
        ...
        console.log(`[getFeaturedTools] Found ${featuredToolsRecords.length} total active and featured tools in Airtable.`);
        ...
        console.log(`[getFeaturedTools] Randomly selected ${mappedTools.length} tools for display: ${mappedTools.map(t => t.name).join(', ')}`);
        console.log(`--- [getFeaturedTools] END ---`);
