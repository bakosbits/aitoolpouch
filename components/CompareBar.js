...
export default function CompareBar({ compareList = [], toggleCompare }) {
    console.log("CompareBar rendered. Current compareList length:", compareList.length);
    if (compareList.length === 0) {
        console.log("CompareBar hidden: compareList is empty.");
        return null;
    }
        ...
        <div className="fixed top-0 left-0 right-0 z-50 bg-backgroundDark border-b border-accentGreen p-4 flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-4 flex-wrap">
                ...
                {compareList.map((tool) => (
                    console.log(`Tool added to CompareBar: ${tool.Name} (ID: ${tool.id})`),
                    <div
                        key={tool.id}
                    ...
                    >
                        ...
                        <button
                            onClick={() => {
                                console.log(`Attempting to remove tool from CompareBar: ${tool.Name} (ID: ${tool.id})`);
                                toggleCompare(tool);
                            }}
            ...
            {compareList.length === 2 &&
                compareList[0].Slug &&
                compareList[1].Slug && (
                    console.log(`"Compare Now" link visible for: ${compareList[0].Name} and ${compareList[1].Name}`),
                    <Link
                        ...
                        onClick={() => {
                            console.log(`"Compare Now" button clicked. Comparing: ${compareList[0].Name} vs ${compareList[1].Name}`);
                        }}
                    >
